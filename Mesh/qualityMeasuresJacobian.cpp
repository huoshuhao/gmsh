...
//  std::cout << "" << min << " [" << min2 << "," << minL2 << "] " << max << " [" << maxL2 << "," << max2 << "] " << std::endl;
  ...
  _CoeffDataJac::_CoeffDataJac(fullVector<double> &v,
      ...
      : _CoeffData(depth), _coeffs(v.getDataPtr(), v.size()), _bfs(bfs), _coeffs2(coeffs2)
  {
    if (!v.getOwnData()) {
  ...
  void _CoeffDataJac::getSubCoeff(std::vector<_CoeffData *> &v) const
  {
    ...
    for(int i = 0; i < _bfs->getNumDivision(); i++) {
      ...
      _CoeffDataJac *newData = new _CoeffDataJac(coeff, _bfs, _depth + 1, sub[i]);
      v.push_back(newData);
    }
  }
  ...
  double _CoeffDataIGE::_computeLowerBound2() const
  {
    ...
    computeCoeffLengthVectors_(m, v, _type);
